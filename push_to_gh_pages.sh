...
before_script:
  - echo -e "Host github.com\n\tStrictHostKeyChecking yes\y" >> ~/.ssh/config
if [ "$POST_BUILD" == "true" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ]; then
        ...
        git clone --quiet ssh://git@github.com/alefebvre/myrepositories.git repo > /dev/null || error_exit "Error cloning the repository";
